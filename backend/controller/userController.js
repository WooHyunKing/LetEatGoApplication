...
const user = {
    ...
    updateMade: async(req, res, err) => {
        try{
            ...
            if(deleteFood){
                ...
                return res.json({statusCOde: CODE.SUCCESS, msg:"만들어본 음식을 업데이트하였습니다."});
            }else{
    ...
    delete: async(req, res) => {
        try{
            ...
            if(!deleteFlag) return res.json({statusCode: CODE.FAIL, msg:"해당 회원을 찾을 수 없습니다."});
            return res.json({statusCode: CODE.SUCCESS, msg:"해당 회원을 삭제하였습니다."});
        }catch(error){
            console.error(error);
            return res.json({statusCode: CODE.FAIL, msg:"db 오류"});
        }
